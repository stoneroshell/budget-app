...
export function DemoDeleteBudgetButton({ budgetId }: { budgetId: string }) {
  const router = useRouter();
  const { deleteBudget } = useDemo();
    ...
    <>
      ...
      {showConfirm && (
        <div
          ...
          aria-modal="true"
          role="dialog"
          aria-labelledby="demo-delete-budget-title"
          onClick={() => {
            setShowConfirm(false);
            setError(null);
          }}
        >
          ...
          >
            <h2
              id="demo-delete-budget-title"
              className="mb-4 text-center text-lg font-medium text-white"
            >
              Are you sure you want to delete this budget?
            </h2>
            <p className="mb-4 text-center text-sm text-charcoal-300">
              This will remove this month&apos;s budget and all its expenses from
              the demo. You can start fresh by closing the tab and reopening.
            </p>
            {error && (
              <p
