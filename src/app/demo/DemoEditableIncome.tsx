...
export function DemoEditableIncome({
  budgetId,
  income,
}: {
  budgetId: string;
  ...
  const { updateBudgetIncome } = useDemo();
  ...
  function handleSave() {
    ...
    setError(null);
    setPending(true);
    const result = updateBudgetIncome(budgetId, parsed);
    setPending(false);
    if (result.error) {
      ...
      <div className="relative flex flex-col items-center">
        <div className="flex items-center justify-center gap-2">
          <input
            ...
            aria-invalid={!!error}
            aria-describedby={error ? "demo-income-error" : undefined}
        ...
        {error && (
          <p id="demo-income-error" className="mt-1 text-center text-sm text-accent-rose-400" role="alert">
            {error}
          </p>
        )}
      </div>
