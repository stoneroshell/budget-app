...
type Supercategory = "needs" | "wants";

export function DemoExpenseList({
  expenses,
  categories,
  budgetId,
}: {
  ...
  const [openComboboxExpenseId, setOpenComboboxExpenseId] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [focusedIndex, setFocusedIndex] = useState(0);
  const [expenseIdForNewCategory, setExpenseIdForNewCategory] = useState<string | null>(null);
  ...
  function handleDeleteExpense(expenseId: string) {
    ...
    if (!result?.error) {
      // Context updated; parent will re-render with new expenses
    }
  }

  function handleCategoryChange(expenseId: string, categoryId: string | null) {
    const result = updateExpenseCategory(expenseId, categoryId);
    if (!result?.error) {
      // Context updated
    }
  }
  ...
  function handleConfirmDelete() {
    ...
    const result = deleteOrHideCategory(deleteConfirmCategoryId);
  ...
  async function handleCreateCategory(e: React.FormEvent) {
    ...
    if (result?.id && expenseIdForNewCategory) {
      updateExpenseCategory(expenseIdForNewCategory, result.id);
    }
    ...
    <>
          ...
          return (
            ...
            >
              ...
              <div className="flex min-w-0 flex-1 items-center gap-2">
                ...
                <div
                  ref={openComboboxExpenseId === e.id ? comboboxRef : undefined}
                ...
                >
                  {(() => {
                    ...
                    function handleComboboxKeyDown(ev: React.KeyboardEvent<HTMLInputElement>) {
                      if (!isOpen) return;
                      if (ev.key === "ArrowDown") {
                        ev.preventDefault();
                        ...
                        ev.preventDefault();
                        const safeIndex = Math.min(focusedIndex, optionCount - 1);
                        if (safeIndex === 0) {
      ...
      {showAddCategory && (
        <div
          ...
          aria-modal="true"
          role="dialog"
          aria-labelledby="demo-add-category-title"
          onClick={() => {
            setShowAddCategory(false);
            setAddError(null);
            setExpenseIdForNewCategory(null);
      ...
      {deleteConfirmCategoryId && (
        <div
          ...
          aria-modal="true"
          role="dialog"
          aria-labelledby="demo-delete-category-title"
          onClick={() => {
            setDeleteConfirmCategoryId(null);
            setDeleteError(null);
          }}
