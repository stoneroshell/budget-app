...
import { DemoCsvImportModal } from "./DemoCsvImportModal";
import { useDemo } from "@/contexts/DemoContext";
import type { Category } from "@/types/database";

type BudgetSummary = { id: string; month: number; year: number };

type DemoCsvImportTriggerProps = {
  categories: Category[];
  budgets: BudgetSummary[];
};

export function DemoCsvImportTrigger({ categories, budgets }: DemoCsvImportTriggerProps) {
  const [open, setOpen] = useState(false);
  const { importExpensesFromCsv } = useDemo();
    ...
    <>
      ...
      {open && (
        <DemoCsvImportModal
          ...
          onImport={importExpensesFromCsv}
        />
      )}
    </>
  );
