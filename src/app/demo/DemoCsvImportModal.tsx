...
type DemoCsvImportModalProps = {
  categories: Category[];
  budgets: BudgetSummary[];
  onClose: () => void;
  onImport: (rows: CsvImportRow[]) => { error?: string; imported?: number; skippedNoBudget?: number };
};
...
export function DemoCsvImportModal({
  categories,
  budgets,
  onClose,
  onImport,
}: DemoCsvImportModalProps) {
  ...
  const budgetSet = new Set(budgets.map((b) => `${b.month},${b.year}`));
  ...
  const buildPreviewRows = useCallback((): PreviewRow[] => {
    ...
    for (const row of parseResult.rows) {
      ...
      if (!descriptionRaw) err = "Missing description";
      else if (amount === null) err = "Invalid amount";
      else if (month === null) err = "Invalid month";
      else if (year === null) err = "Invalid year";
      else if (!budgetSet.has(`${month},${year}`))
        err = "No budget for this month";
  ...
  const handleImport = () => {
    ...
    setError(null);
    setPending(true);
    const payload: CsvImportRow[] = validRows.map((r) => ({
      month: r.month,
      year: r.year,
    ...
    <div className="rounded-2xl border border-charcoal-500 bg-charcoal-900 p-6 shadow-xl">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h2 id="demo-import-csv-title" className="font-display text-xl font-light text-white tracking-tight">
            Import CSV
          </h2>
      ...
      {step === "mapping" && parseResult && (
        <div className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-sm text-charcoal-300">Amount</label>
              <select
            ...
            <div>
              <label className="mb-1 block text-sm text-charcoal-300">Description</label>
              <select
            ...
            <div>
              <label className="mb-1 block text-sm text-charcoal-300">Month (3-letter)</label>
              <select
                value={monthColIndex}
            ...
            <div>
              <label className="mb-1 block text-sm text-charcoal-300">Year</label>
              <select
            ...
            {yearColIndex === null && (
              <div className="sm:col-span-2">
                <label className="mb-1 block text-sm text-charcoal-300">Default year</label>
                <select
                  value={defaultYear}
            ...
            <table className="w-full text-left text-sm">
              <thead className="sticky top-0 bg-charcoal-900 text-charcoal-400">
                <tr>
                  ...
                  {skippedErrors > 0 && <th className="px-3 py-2">Issue</th>}
                </tr>
              </thead>
              <tbody>
                {previewRows.map((r, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-1.5 text-charcoal-300">{r.categoryName}</td>
                    {skippedErrors > 0 && (
                      <td className="px-3 py-1.5 text-xs text-accent-rose-400/90">
                        {r.error ?? ""}
                      </td>
                    )}
                  </tr>
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="demo-import-csv-title"
    >
